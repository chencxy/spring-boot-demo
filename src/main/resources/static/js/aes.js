 ...
 */
function Cipher(input, w) {    // main Cipher function [搂5.1]
  var Nb = 4;               // block size (in words): no of columns in state (fixed at 4 for AES)
  var Nr = w.length/Nb - 1; // no of rounds: 10/12/14 for 128/192/256-bit keys

  var state = [[],[],[],[]];  // initialise 4xNb byte-array 'state' with input [搂3.4]
  ...
  var output = new Array(4*Nb);  // convert state to 1-d array before returning [搂3.4]
  for (var i=0; i<4*Nb; i++) output[i] = state[i%4][Math.floor(i/4)];
  return output;
}


function SubBytes(s, Nb) {    // apply SBox to state S [搂5.1.1]
  for (var r=0; r<4; r++) {
...
function ShiftRows(s, Nb) {    // shift row r of state S left by r bytes [搂5.1.2]
  var t = new Array(4);
  for (var r=1; r<4; r++) {
...
function MixColumns(s, Nb) {   // combine bytes of each col of state S [搂5.1.3]
  for (var c=0; c<4; c++) {
    var a = new Array(4);  // 'a' is a copy of the current column from 's'
    var b = new Array(4);  // 'b' is a鈥02} in GF(2^8)
    for (var i=0; i<4; i++) {
      a[i] = s[i][c];
      b[i] = s[i][c]&0x80 ? s[i][c]<<1 ^ 0x011b : s[i][c]<<1;
    }
    // a[n] ^ b[n] is a鈥03} in GF(2^8)
...
function AddRoundKey(state, w, rnd, Nb) {  // xor Round Key into state S [搂5.1.4]
  for (var r=0; r<4; r++) {
...
function KeyExpansion(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [搂5.2]
  var Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
...
// Sbox is pre-computed multiplicative inverse in GF(2^8) used in SubBytes and KeyExpansion [搂5.1.1]
...
// Rcon is Round Constant used for the Key Expansion [1st col is 2^(r-1) in GF(2^8)] [搂5.2]
...
function AESEncryptCtr(plaintext, password, nBits) {
  ...
  // initialise counter block (NIST SP800-38A 搂B.2): millisecond time-stamp for nonce in 1st 8 bytes,
